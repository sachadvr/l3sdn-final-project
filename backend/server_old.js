...
app.get('/api/users', verifyToken, (req, res) => {
  fs.readFile('user.json', 'utf8', (err, data) => {
    ...
    try {
      ...
      res.json(users.map(u => ({id: u.id, username: u.username, role: u.role, manager_id: u.manager_id })))
    } catch (parseError) {
      res.status(500).json({ message: 'Impossible de lire les utilisateurs' })
    }
  })
...
app.get('/api/interviews', verifyToken, (req, res) => {
  fs.readFile('interviews.json', 'utf8', (err, data) => {
    if (err) {
      return res.status(500).json({ message: 'Impossible de lire les entretiens' })
    }
    ...
    try {
      ...
      if (req.query.manager_id) {
        res.json(interviews.filter(i => i.manager_id == req.query.manager_id))
        return
      }
      res.json(interviews)
    } catch (parseError) {
...
app.get('/api/objectives/:userid', verifyToken, (req, res) => {
  fs.readFile('objectifs.json', 'utf8', (err, data) => {
    if (err) {
      return res.status(500).json({ message: 'Impossible de lire les objectifs' })
    }

    try {
      const objectives = JSON.parse(data)
      if (req.query.year) {
        res.json(objectives.filter(i => i.user_id == req.params.userid && i.date.startsWith(req.query.year)))
        return
      }
      res.json(objectives.filter(i => i.user_id == req.params.userid))
    } catch (parseError) {
      res.status(500).json({ message: 'Impossible de lire les objectifs' })
    }
  })
})
